...
![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\锁分类概览.png)
...
![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\乐观锁 VS 悲观锁.png)
...
之前提到java.util.concurrent包中的原子类，就是通过CAS来实现了乐观锁，那么我们进入原子类AtomicInteger的源码

![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\原子类AtomicInteger.png)
...
![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\自旋锁 VS 适应性自旋锁.png)

自旋锁本身是有缺点的，它不能代替阻塞。自旋等待虽然避免了线程切换的开销，但它要占用处理器时间。如果锁被占用的时间很短，自旋等待的效果就会非常好。反之，如果锁被占用的时间很长，那么自旋的线程只会白浪费处理器资源。所以，自旋等待的时间必须要有一定的限度，如果自旋超过了限定次数（默认是10次，可以使用-XX:PreBlockSpin来更改）没有成功获得锁，就应当挂起线程。

自旋锁的实现原理同样也是CAS，AtomicInteger中调用unsafe进行自增操作的源码中的do-while循环就是一个自旋操作，如果修改数值失败则通过循环来执行自旋，直至修改成功。

![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\自旋锁的实现原理.png)
...
![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\32位虚拟机对象头分配.jpeg)
...
![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\锁升级流程.png)

综上，偏向锁通过对比Mark Word解决加锁问题，避免执行CAS操作。而轻量级锁是通过用CAS操作和自旋来解决加锁问题，避免线程阻塞和唤醒而影响性能。重量级锁是将除了拥有锁的线程以外的线程都阻塞。



关于获取锁释放锁的详解https://developer.51cto.com/art/201910/604405.htm
...
![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\公平锁 VS 非公平锁1.png)

如上图所示，假设有一口水井，有管理员看守，管理员有一把锁，只有拿到锁的人才能够打水，打完水要把锁还给管理员。每个过来打水的人都要管理员的允许并拿到锁之后才能去打水，如果前面有人正在打水，那么这个想要打水的人就必须排队。管理员会查看下一个要去打水的人是不是队伍里排最前面的人，如果是的话，才会给你锁让你去打水；如果你不是排第一的人，就必须去队尾排队，这就是公平锁。

但是对于非公平锁，管理员对打水的人没有要求。即使等待队伍里有排队等待的人，但如果在上一个人刚打完水把锁还给管理员而且管理员还没有允许等待队伍里下一个人去打水时，刚好来了一个插队的人，这个插队的人是可以直接从管理员那里拿到锁去打水，不需要排队，原本排队等待的人只能继续等待。如下图所示：

![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\公平锁 VS 非公平锁2.png)

接下来我们通过ReentrantLock的源码来讲解公平锁和非公平锁。

![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\ReentrantLock的源码.png)

根据代码可知，ReentrantLock里面有一个内部类Sync，Sync继承AQS（AbstractQueuedSynchronizer），添加锁和释放锁的大部分操作实际上都是在Sync中实现的。它有公平锁FairSync和非公平锁NonfairSync两个子类。ReentrantLock默认使用非公平锁，也可以通过构造器来显示的指定使用公平锁。

下面我们来看一下公平锁与非公平锁的加锁方法的源码:

![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\公平锁与非公平锁的加锁方法.png)

通过上图中的源代码对比，我们可以明显的看出公平锁与非公平锁的lock()方法唯一的区别就在于公平锁在获取同步状态时多了一个限制条件：hasQueuedPredecessors()。

![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\hasQueuedPredecessors().png)
...
![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\可重入锁.png)

但如果是非可重入锁的话，此时管理员只允许锁和同一个人的一个水桶绑定。第一个水桶和锁绑定打完水之后并不会释放锁，导致第二个水桶不能和锁绑定也无法打水。当前线程出现死锁，整个等待队列中的所有线程都无法被唤醒。



![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\非可重入锁.png)
...
![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\ReentrantReadWriteLock的部分源码.png)
...
![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\读写锁state字段.png)
...
![](D:\java oracle\学习资料\各种学习整理文档（重要）\java 基础\java锁机制\assert\互斥锁ReentrantLock中公平锁和非公平锁的加锁源码.png)
