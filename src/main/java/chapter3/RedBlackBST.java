...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    private Node put(Node h, Key key, Value val) {
        ...
        // 如果进入这个if条件,这里有一个隐藏条件:h节点一定是black.
        // 首先put(Key key, Value val)方法把根节点置黑了。
        // 红黑树是没有一个节点和两条两个红连接相连（当前节点和子节点都是红连接或者两个子节点都是红连接）。
        // 所以如果左右节点都是红色，一定是新增节点挂在h节点下了。
        // 既然先前存在一个左或者右子节点红色节点，则当前h节点不可能是红色节点了。
    ...
    public static void main(String[] args) {
        ...
        st.put("a1", 1);
        st.put("b1", 1);
        System.out.println("st.root.key() = " + st.root.key);
        System.out.println("st.root.left.key() = " + st.root.left.key);
        st.deleteMin();
